...
Сравнительный анализ трёх алгоритмов приближённого поиска ближайших соседей (Approximate Nearest Neighbor, ANN) на
датасете SIFT1M с использованием библиотеки [FAISS](https://github.com/facebookresearch/faiss).

---

## Датасет

**SIFT1M** — стандартный датасет для оценки ANN-алгоритмов, состоящий из дескрипторов SIFT (Scale-Invariant Feature
Transform):
...
| Ground truth       | 100 ближайших соседей на запрос (вычислено brute-force) |
...
Длина бинарной подписи — количество случайных гиперплоскостей, через которые проецируется вектор.
...
- **Recall** растёт с увеличением nbits: больше гиперплоскостей - более точная бинарная подпись, меньше ложных коллизий
- **QPS** падает: сравнение более длинных подписей занимает больше времени
...
*Зафиксировано: efConstruction=200, efSearch=64*
...
*Зафиксировано: M=32, efSearch=128*
...
*Зафиксировано: M=32, efConstruction=200*

**Влияние `efSearch`:**

- **Recall** быстро растёт от 0.62 до 0.99: увеличение ширины поиска позволяет не застревать в локальных минимумах
  графа
...
1. Вектор размерности d=128 разрезается на `m_pq` подвекторов
2. Для каждого подвектора обучается кодбук из 256 центроидов (8 бит на подвектор)
3. Каждый подвектор заменяется 1 байтом - индексом ближайшего центроида

**Пример сжатия:** вектор из 128 float32 = 512 байт. При m_pq=8 сжимается до 8 байт - **в 64 раза**.

При поиске расстояние вычисляется приближённо через ADC (Asymmetric Distance Computation): для каждого подвектора
запроса заранее считается таблица расстояний до 256 центроидов, а итоговое расстояние - сумма `m_pq` table lookups.
...
*Зафиксировано: nlist=1024, m_pq=32*
...
|------|------------|-----------------|------------------------------|
| 8    | 0.452      | 15.5            | 128 float32 -> 8 байт (64x)  |
| 16   | 0.619      | 23.1            | 128 float32 -> 16 байт (32x) |
| 32   | 0.749      | 38.4            | 128 float32 -> 32 байт (16x) |

*Зафиксировано: nlist=256, nprobe=10*
...
#### Параметр: `nlist` - число кластеров Вороного
...
*Зафиксировано: m_pq=32, nprobe=10*
...
- **HNSW** (синие точки) - доминирует в правой верхней части: высокий recall (0.9-1.0) при высоком QPS (3K-100K). Лучший
  алгоритм, когда есть достаточно памяти
- **IVF+PQ** (зелёные треугольники) - широкий разброс. При малом nprobe может достигать >200K QPS, но при низком recall.
  Оптимальные конфигурации дают recall 0.7-0.8 при 3K-50K QPS
- **LSH** (красные квадраты) - всего 5 точек, все в нижней левой части. Recall не превышает 0.70, а QPS не выше 3 861
...
| HNSW     | 96           | 0.41 — 1.00      | 2 171 — 110 089 | 565 — 992       |
...
| Ограниченная RAM (миллиарды векторов) | IVF+PQ                 | nlist=1024, m_pq=32, nprobe=20         |
| Максимальный QPS, recall не критичен  | IVF+PQ                 | nlist=1024, m_pq=8, nprobe=1           |
| Быстрое прототипирование              | LSH                    | nbits=512                              |

Результаты сохраняются в `results/benchmark_results.json`, графики - в `results/*.png`.
